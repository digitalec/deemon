...
class Database(object):
    ...
    def add_new_releases(self, values):
        self.new_transaction()
        sql = (
            f"INSERT OR REPLACE INTO releases ('artist_id', 'artist_name', 'album_id', 'album_name', 'album_release', "
    ...
    def remove_specific_releases(self, values):
        self.query(f"DELETE FROM releases WHERE album_release > :tm_date AND profile_id = {config.profile_id()}",
                   values)
